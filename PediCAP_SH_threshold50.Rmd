---
title: "PediCAP_analysis_script_threshold_50K"
author: "SH&JPRR"
date: "2024-05-27"
...
write.csv(genus_cont_removed, "~/data/Servaas/PediCAP/R_output/Decontam/genus_cont_removed_50K.csv")
...
write.csv(species_cont_removed, "~/data/Servaas/PediCAP/R_output/Decontam/species_cont_removed_50K.csv")
...
genus_merged <- genus_merged[genus_merged$Threshold50 == "TRUE",]
species_merged <- species_merged[species_merged$Threshold50 == "TRUE",]
...
# otu.rarecurve1k_species = rarecurve(rarefaction_species, step = 1000, label = FALSE, cex = 0.6,  xlab= "Sequences per sample with step 1000", ylab= "OTU's") # to see if it affects low read samples
...
# otu.rarecurve1k_genus = rarecurve(rarefaction_genus, step = 1000, label = FALSE, cex = 0.6,  xlab= "Number of genus reads with step 1000", ylab= "OTU's") # to see if it affects low read samples

# otu.rarecurve10k_genus = rarecurve(rarefaction_genus, step = 10000, label = FALSE, cex = 0.6,  xlab= "Number of genus reads with step 10000", ylab= "OTU's")
...
# genus normally distributed for 50K threshold
```
...
adonis2(species_betabray_resampled ~ Extraction * Kit * Colour * Site * PCR * Reads * Bacterial_reads * Percentage_host_rem * Percentage_trimmed * Qubit, data = metadata_pcoa, permutations = 999)
...
adonis2(genus_betabray_resampled ~ Extraction * Kit * Colour * Site * PCR * Reads * Bacterial_reads * Percentage_host_rem * Percentage_trimmed * Qubit, data = metadata_pcoa, permutations = 999)
...
metadata_pcoa2 <- metadata_pcoa[1:58,]
...
Dive_site_species <- ggplot(metadata_pcoa2, aes(x = Site, y = Shannon)) +
   ...
   labs(title = "Species Shannon diversity per country of origin", tag = "B") +
...
Dive_colour_genus <- ggplot(metadata_pcoa, aes(x = Colour, y = Shannon)) +
   ylim(0,6) +
   geom_jitter(position = position_jitterdodge(jitter.width = 0), data = shannon_analysis, aes(color = Colour, size = 1)) +
   geom_point(position = position_jitterdodge(jitter.width = 0), aes(color = Colour, alpha = 0.5), show.legend = FALSE) +
   geom_errorbar(position = position_jitterdodge(jitter.width = 0), data = shannon_analysis, aes(color = Colour, ymin = Shannon+sd, ymax = Shannon+sd), width = 0.15, show.legend = FALSE) +
...
write.csv(dds_species_extr_results_sig1, file= "~/data/Servaas/PediCAP/R_output/DeSEQ2/species_site_t20.csv")

# ## Species colour yes/no
# dds_species_extr2 <- DESeqDataSetFromMatrix(countData = t(species), colData = as.data.frame(unclass(metadata), stringsAsFactors = TRUE), design = ~ as.factor(Colour))
...
# ## Species kit 1 and 2
# dds_species_stextr8 <- DESeqDataSetFromMatrix(countData = t(species), colData = as.data.frame(unclass(metadata), stringsAsFactors = TRUE), design = ~ as.factor(Kit))
# dds_species_stextr8 <- DESeq(dds_species_stextr8)
# dds_species_stextr_results8 <- results(dds_species_stextr8, contrast = c("Kit", "1", "2"))
...
# # write.csv(dds_species_stextr_results_sig8, file= "~/data/Servaas/PediCAP/DeSEQ8/species_threshold20_kit12_t20.csv")
# 
...
# dds_species_stextr26 <- DESeqDataSetFromMatrix(countData = t(species), colData = as.data.frame(unclass(metadata), stringsAsFactors = TRUE), design = ~ Kit + Threshold500)
# dds_species_stextr26 <- DESeq(dds_species_stextr26)
# dds_species_stextr_results26 <- results(dds_species_stextr26, contrast = c("Kit", "1", "2", "3"))
# sum(dds_species_stextr_results26$padj <0.1, na.rm = TRUE)
# dds_species_stextr_results_sig26 <- dds_species_stextr_results26[which(dds_species_stextr_results26$padj < 0.1), ]
# # write.csv(dds_species_stextr_results_sig26, file= "~/data/Servaas/PediCAP/DeSEQ2/species_threshold500_kit_t20.csv")
# 
...
# 
# ## Species site and 20K threshold
# dds_species_stextr10 <- DESeqDataSetFromMatrix(countData = t(species), colData = as.data.frame(unclass(metadata), stringsAsFactors = TRUE), design = ~ Site + Threshold20)
...
# # write.csv(dds_species_stextr_results_sig10, file= "~/data/Servaas/PediCAP/DeSEQ10/species_threshold20_site_t20.csv")
# 
...
## genus kit and 20K threshold
dds_genus_stextr8 <- DESeqDataSetFromMatrix(countData = t(genus), colData = as.data.frame(unclass(metadata), stringsAsFactors = TRUE), design = ~ Kit + Threshold20)
...
# write.csv(dds_genus_stextr_results_sig8, file= "~/data/Servaas/PediCAP/DeSEQ8/genus_threshold20_kit_t20.csv")
...
dds_genus_stextr26 <- DESeqDataSetFromMatrix(countData = t(genus), colData = as.data.frame(unclass(metadata), stringsAsFactors = TRUE), design = ~ Kit + Threshold500)
dds_genus_stextr26 <- DESeq(dds_genus_stextr26)
dds_genus_stextr_results26 <- results(dds_genus_stextr26, contrast = c("Kit", "1", "2", "3"))
sum(dds_genus_stextr_results26$padj <0.1, na.rm = TRUE)
dds_genus_stextr_results_sig26 <- dds_genus_stextr_results2[which(dds_genus_stextr_results26$padj < 0.1), ]
# write.csv(dds_genus_stextr_results_sig26, file= "~/data/Servaas/PediCAP/DeSEQ2/genus_threshold500_kit_t20.csv")
...
## genus site and 20K threshold
dds_genus_stextr10 <- DESeqDataSetFromMatrix(countData = t(genus), colData = as.data.frame(unclass(metadata), stringsAsFactors = TRUE), design = ~ Site + Threshold20)
...
# write.csv(dds_genus_stextr_results_sig10, file= "~/data/Servaas/PediCAP/DeSEQ10/genus_threshold20_site_t20.csv")
