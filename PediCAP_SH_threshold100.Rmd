---
title: "PediCAP_analysis_script_threshold_100K"
author: "SH&JPRR"
date: "2024-05-27"
...
write.csv(genus_cont_removed, "~/data/Servaas/PediCAP/R_output/Decontam/genus_cont_removed_100K.csv")
...
write.csv(species_cont_removed, "~/data/Servaas/PediCAP/R_output/Decontam/species_cont_removed_100K.csv")
...
genus_merged <- genus_merged[genus_merged$Threshold100 == "TRUE",]
species_merged <- species_merged[species_merged$Threshold100 == "TRUE",]
...
# otu.rarecurve1k_species = rarecurve(rarefaction_species, step = 1000, label = FALSE, cex = 0.6,  xlab= "Sequences per sample with step 1000 with threshold 100K", ylab= "OTU's") # to see if it affects low read samples
# otu.rarecurve1k_species = rarecurve(rarefaction_species, step = 1000, label = T, cex = 0.6,  xlab= "Sequences per sample with step 1000 with threshold 100K", ylab= "OTU's")
...
# otu.rarecurve1k_species = rarecurve(rarefaction_species, step = 10000, label = T, cex = 0.6,  xlab= "Sequences per sample with step 10000 with threshold 100K", ylab= "OTU's")
...
# otu.rarecurve1k_genus = rarecurve(rarefaction_genus, step = 1000, label = T, cex = 0.6,  xlab= "Number of genus reads with step 1000 with threshold 100K", ylab= "OTU's")
# otu.rarecurve10k_genus = rarecurve(rarefaction_genus, step = 10000, label = FALSE, cex = 0.6,  xlab= "Number of genus reads with step 10000 with threshold 100K", ylab= "OTU's")
# otu.rarecurve10k_genus = rarecurve(rarefaction_genus, step = 10000, label = T, cex = 0.6,  xlab= "Number of genus reads with step 10000 with threshold 100K", ylab= "OTU's")
...
# genus normally distributed for 100K threshold
```
...
metadata_pcoa2 <- metadata_pcoa[1:51,]
...
Dive_site_genus <- ggplot(metadata_pcoa2, aes(x = Site, y = Shannon_genus)) +
   ...
   xlab(NULL) +
   guides(color = guide_legend(title = "Sample origin", title.position = "top", title.hjust = 0.5, label.hjust = 0.5), size = "none") +
   labs(title = "Genus Shannon diversity per country of origin", tag = "B") +
   theme(text = element_text(size=10), plot.title = element_text(size = 10, hjust = 0.5), axis.text.y = element_text(size = 10), axis.ticks.x = element_blank(), axis.text.x = element_text(size = 10))
