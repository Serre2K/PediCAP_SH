---
title: "PediCAP_analysis_script_threshold_20K"
author: "SH&JPRR"
date: "2024-05-27"
...
write.csv(genus_cont_removed, "~/data/Servaas/PediCAP/R_output/Decontam/genus_cont_removed_20K.csv")
...
write.csv(species_cont_removed, "~/data/Servaas/PediCAP/R_output/Decontam/species_cont_removed_20K.csv")
...
genus_merged <- genus_merged[genus_merged$Threshold20 == "TRUE",]
species_merged <- species_merged[species_merged$Threshold20 == "TRUE",]
...
# both non normally distributed for 20k threshold
```
...
# TukeyHSD(anova_extr)
# 
# anova_colour = aov(Shannon_genus ~ Colour, metadata)
# TukeyHSD(anova_colour)
# 
# anova_pcr = aov(Shannon_genus ~ PCR, metadata)
# TukeyHSD(anova_pcr)
# 
# anova_kit = aov(Shannon_genus ~ Kit, metadata)
# TukeyHSD(anova_kit)
...
metadata_pcoa2 <- metadata_pcoa[1:72,]
...
Dive_site_genus <- ggplot(metadata_pcoa2, aes(x = Site, y = Shannon_genus)) +
   ...
   xlab(NULL) +
   guides(color = guide_legend(title = "Sample origin", title.position = "top", title.hjust = 0.5, label.hjust = 0.5), size = "none") +
   labs(title = "Shannon diversity at genus level country", tag = "B") +
   theme(text = element_text(size=10), plot.title = element_text(size = 10, hjust = 0.5), axis.text.y = element_text(size = 10), axis.ticks.x = element_blank(), axis.text.x = element_text(size = 10))
...
# library(tidyr)
# phylum_2_SA <- read.csv("~/data/Servaas/PediCAP/R_input/bracken_uhgg_phylum_summary_num_SA.csv", check.names = FALSE)
...
#   ggtitle("Relative phylum abundance in South Africa") +
...
# # go into excel, filter on phylum, then select 10 most abundant phyla, group rest under other. Then calculate percentage on them based on the root.
# library(tidyr)
# phylum_2_Ug<- read.csv("~/data/Servaas/PediCAP/R_input/bracken_uhgg_phylum_summary_num_Uganda.csv", check.names = FALSE)
...
#   theme(axis.text.x = element_text(angle=90)) +
#   guides(x = guide_axis(angle = 90)) +
#   ggtitle("Relative phylum abundance in Uganda") +
#   labs(x= "Samples", fill= "Phylum", y= "Relative Abundance") +
#   scale_fill_brewer(palette = "Paired")
# phylum_UG
```
...
# # go into excel, filter on phylum, then select 10 most abundant phyla, group rest under other. Then calculate percentage on them based on the root.
# library(tidyr)
# phylum_average <- read.csv("~/data/Servaas/PediCAP/R_input/bracken_uhgg_phylum_summary_num_average.csv", check.names = FALSE)
...
#   theme(axis.text.x = element_text(angle=90)) +
#   guides(x = guide_axis(angle = 90)) +
#   ggtitle("Relative phylum abundance per country average") +
#   labs(x= "Samples", fill= "Phylum", y= "Relative Abundance") +
#   scale_fill_brewer(palette = "Paired")
# phylum_average_plot
```
...
# go into excel, filter on phylum, then select 10 most abundant phyla, group rest under other. Then calculate percentage on them based on the root.
library(tidyr)
phylum_negatives <- read.csv("~/data/Servaas/PediCAP/R_input/bracken_uhgg_phylum_summary_num_neg.csv", check.names = FALSE)
# phylu_2_average <- phylum_2[1:11,]
# phylum_2_rem <- phylu_2_rem[,c(2,4,6,8,10,12,14,16,18,19)]
# colnames(phylum_2_rem)[1:9] <- c("Stool1-MagMAX","Stool2-MagMAX","Stool1-MPBio","Stool2-MPBio","Swab1-MagMAX","Swab2-MagMAX","Swab1-MPBio","Swab2-MPBio","Zymo")
...
phylum_negative_plot <- ggplot(aes(x=variable, y=value, fill=name), data=phyla_data_negative) +
  ...
  theme(axis.text.x = element_text(angle=90)) +
  guides(x = guide_axis(angle = 90)) +
  ggtitle("Relative phylum abundance of negative samples") +
  labs(x= "Samples", fill= "Phylum", y= "Relative Abundance") +
  scale_fill_brewer(palette = "Paired")
phylum_negative_plot
```

```{r taxonomy plot negative samples stacked}
...
dds_species_extr2$Colour <- relevel(dds_species_extr2$Site, "South_Africa")
